...
class DatabaseHandler:
    def __init__(
        ...
        self.db_cnx = db_cnx(config=config, logger=self.logger)
        self.db_operations = db_operations(
            db_connector=self.db_cnx,
            logger=self.logger
        )
        ...
        try:
            ...
            if truncate:
                self.db_operations.execute_query(
                    query="TRUNCATE TABLE core.bets"
                )
            ...
            for row in data:
                ...
                row = tuple(
                    [x if not isinstance(x, datetime) else x for x in row]
                )
