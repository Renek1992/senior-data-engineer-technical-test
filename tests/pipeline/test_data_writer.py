...
def describe_DatabaseHandler():
    ...
    def db_handler(mock_logger, mock_config):
        with patch('src.pipeline.data_writer.db_handler.DatabaseConnector') as mock_db_cnx:
            with patch('src.pipeline.data_writer.db_handler.DatabaseOperations') as mock_db_operations:
    ...
    def describe_initialization():
        def it_initializes_with_child_logger(mock_logger, mock_config):
            with patch('src.pipeline.data_writer.db_handler.DatabaseConnector'), \
        ...
        def it_creates_db_connector_instance(mock_logger, mock_config):
            with patch('src.pipeline.data_writer.db_handler.DatabaseConnector') as mock_db_cnx_class, \
                 patch('src.pipeline.data_writer.db_handler.DatabaseOperations'):
                handler = DatabaseHandler(logger=mock_logger, config=mock_config)
                mock_db_cnx_class.assert_called_once_with(config=mock_config, logger=handler.logger)
        
        def it_creates_db_operations_instance(mock_logger, mock_config):
            with patch('src.pipeline.data_writer.db_handler.DatabaseConnector') as mock_db_cnx_class, \
                 patch('src.pipeline.data_writer.db_handler.DatabaseOperations') as mock_db_ops_class:
                handler = DatabaseHandler(logger=mock_logger, config=mock_config)
                mock_db_ops_class.assert_called_once_with(
                    db_connector=handler.db_cnx, 
    ...
    def describe_run():
        ...
        def it_cleans_null_values_in_data(db_handler, test_df):
            with patch.object(db_handler, '_to_tuples', return_value=[
                ...
                assert any(args[1]['params'] == (1, "test1", datetime(2023, 1, 1)) for args in call_args)
                assert any(args[1]['params'] == (2, None, datetime(2023, 1, 2)) for args in call_args)
                assert any(args[1]['params'] == (3, "test3", datetime(2023, 1, 3)) for args in call_args)
        ...
        def it_truncates_table_when_specified(db_handler, test_df):
            with patch.object(db_handler, '_to_tuples', return_value=[]), \
                ...
                query_arg = first_call[1].get('query') if 'query' in first_call[1] else first_call[0][0]
        ...
        def it_does_not_truncate_table_when_not_specified(db_handler, test_df):
            with patch.object(db_handler, '_to_tuples', return_value=[]), \
                ...
                for call in mock_execute.call_args_list:
                    query_arg = call[1].get('query') if 'query' in call[1] else call[0][0]
        ...
        def it_logs_success_message_with_row_count(db_handler, test_df):
            with patch.object(db_handler, '_to_tuples', return_value=[
                ...
                db_handler.run(test_df, truncate=False)
                db_handler.logger.info.assert_called_with("Successfully wrote 3 rows to raw.bet.")
        
        def it_handles_errors_and_logs_them(db_handler, test_df):
            with patch.object(db_handler, '_to_tuples', side_effect=Exception("Test error")):
                with pytest.raises(Exception) as excinfo:
                    db_handler.run(test_df, truncate=False)
                
                assert "Test error" in str(excinfo.value)
                db_handler.logger.error.assert_called_once()
                assert "Error writing DataFrame to Postgres" in db_handler.logger.error.call_args[0][0]
        
        def it_preserves_datetime_objects(db_handler, test_df):
            test_date = datetime(2023, 1, 1)
            with patch.object(db_handler, '_to_tuples', return_value=[(1, "test", test_date)]), \
                 patch.object(db_handler.db_operations, 'execute_query') as mock_execute:
                db_handler.run(test_df, truncate=False)
